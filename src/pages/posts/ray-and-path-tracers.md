...
I'll breifly talk about ray and path tracing, and some specific changes I've made.
...
I first tackled this problem while learning computer graphics on my own during the pandemic.
...
We start with a camera at some point in that 3D scene, and from the camera, we shoot out rays into the scene.
...
To figure that out, we will see what color the object is, if it is shiny or not, or if it is transparent to name a few.
...
By using C++17's execution policies, I was able to easily create a parallel for-each loop as shown in the following code.
...
I have worked with OpenMPI[^ompi] previously, so that would be a fun exercise to get a few machines together and stress test them.

#### Linear Algebra Approach to Ray-Sphere intersections

Do you like quadratic equations? Really? Something that I haven't liked about teaching ray tracing is that as students we start in fun linear algebra land.
We're thinking of rays shooting across space, then we are presented with the quadratic formula.
I really like the approach to the ray-sphere intersection that is based on geometry and intuition.
Or in other words, I don't find the quadratic equation approach intuitive.

Here is a crude writeup of the general process to solve the ray-sphere intersection with some linear algebra.
...
One thing I will say is that in this approach is that I like how there are so many ways that we can return early out of the intersection test.
...
Path tracing is one proposed way to solve the rendering equation, or try to account for all light interactions.
It is an extension to ray tracing where we randomly sample light paths (think reflection, refraction, shadow rays).
...
<figure>
  <img src="/assets/images/rp-renders/end-scene-out.png" width="512px" alt="Zoomed out demo scene"/>
...
The basics of this is that instead of returning some background color, you would sample a cube map with a texture of an environment.
That way you get to see some sky in reflections.

This does apply to the ray tracing method as well, since you can toggle between which rendering method you want in the scene file.
...
Once I learned about computer graphics, I was sold.
